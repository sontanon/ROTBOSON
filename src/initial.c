...
void initial_guess(rb_context *ctx, double *u)
{
    ...
    if (ctx->readInitialData == 3)
    {
        ...
        double *u_0 = (double *)SAFE_MALLOC((GNUM * ctx->NrTotalInitial * ctx->NzTotalInitial + 1) *
                                            sizeof(double));

        // Read initial data.
        read_single_file_2d(u_0 + 0 * ctx->NrTotalInitial * ctx->NzTotalInitial, ctx->log_alpha_i,
                            ctx->NrTotalInitial, ctx->NzTotalInitial, ctx->NrTotalInitial,
                            ctx->NzTotalInitial, __FILE__, __LINE__);
        read_single_file_2d(u_0 + 1 * ctx->NrTotalInitial * ctx->NzTotalInitial, ctx->beta_i,
                            ctx->NrTotalInitial, ctx->NzTotalInitial, ctx->NrTotalInitial,
                            ctx->NzTotalInitial, __FILE__, __LINE__);
        read_single_file_2d(u_0 + 2 * ctx->NrTotalInitial * ctx->NzTotalInitial, ctx->log_h_i,
                            ctx->NrTotalInitial, ctx->NzTotalInitial, ctx->NrTotalInitial,
                            ctx->NzTotalInitial, __FILE__, __LINE__);
        read_single_file_2d(u_0 + 3 * ctx->NrTotalInitial * ctx->NzTotalInitial, ctx->log_a_i,
                            ctx->NrTotalInitial, ctx->NzTotalInitial, ctx->NrTotalInitial,
                            ctx->NzTotalInitial, __FILE__, __LINE__);
        read_single_file_2d(u_0 + 4 * ctx->NrTotalInitial * ctx->NzTotalInitial, ctx->psi_i,
                            ctx->NrTotalInitial, ctx->NzTotalInitial, ctx->NrTotalInitial,
                            ctx->NzTotalInitial, __FILE__, __LINE__);
        if (ctx->lambda_i)
        {
            read_single_file_2d(u_0 + 5 * ctx->NrTotalInitial * ctx->NzTotalInitial, ctx->lambda_i,
                                ctx->NrTotalInitial, ctx->NzTotalInitial, ctx->NrTotalInitial,
                                ctx->NzTotalInitial, __FILE__, __LINE__);
        }
        else
...
#pragma omp for schedule(static)
                for (i = k; i < ctx->NrTotalInitial; ++i)
                {
                    ...
                    {
                        u_0[5 * ctx->NrTotalInitial * ctx->NzTotalInitial +
                            i * ctx->NzTotalInitial + j] =
                            (exp(2.0 * u_0[3 * ctx->NrTotalInitial * ctx->NzTotalInitial +
                                           i * ctx->NzTotalInitial + j]) -
                             exp(2.0 * u_0[2 * ctx->NrTotalInitial * ctx->NzTotalInitial +
                                           i * ctx->NzTotalInitial + j])) /
                            (r * r);
                    }
                }
            ...
            for (i = 0; i < k; ++i)
            {
                for (j = ctx->ghost_i; j < ctx->NzTotalInitial; ++j)
                {
                    u_0[5 * ctx->NrTotalInitial * ctx->NzTotalInitial + i * ctx->NzTotalInitial +
                        j] = u_0[5 * ctx->NrTotalInitial * ctx->NzTotalInitial +
                                 k * ctx->NzTotalInitial + j];
                }
            }
        }
        ...
        initial_interpolator(u, u_0, ctx->NrTotalInitial - 2 * ctx->ghost_i,
                             ...
                             ctx->dr_i, ctx->dz_i, ctx->NrInterior, ctx->NzInterior, ctx->ghost,
                             ctx->order, ctx->dr, ctx->dz, ctx->w0, ctx->m, ctx->l);
...
#pragma omp for schedule(static)
        ...
        {
            read_single_file_2d(u + 0 * ctx->dim, ctx->log_alpha_i, ctx->NrTotal, ctx->NzTotal,
                                ctx->NrTotalInitial, ctx->NzTotalInitial, __FILE__, __LINE__);
...
#pragma omp for schedule(static)
        ...
        {
            read_single_file_2d(u + 1 * ctx->dim, ctx->beta_i, ctx->NrTotal, ctx->NzTotal,
                                ctx->NrTotalInitial, ctx->NzTotalInitial, __FILE__, __LINE__);
...
#pragma omp for schedule(static)
        ...
        {
            read_single_file_2d(u + 2 * ctx->dim, ctx->log_h_i, ctx->NrTotal, ctx->NzTotal,
                                ctx->NrTotalInitial, ctx->NzTotalInitial, __FILE__, __LINE__);
...
#pragma omp for schedule(static)
        ...
        {
            read_single_file_2d(u + 3 * ctx->dim, ctx->log_a_i, ctx->NrTotal, ctx->NzTotal,
                                ctx->NrTotalInitial, ctx->NzTotalInitial, __FILE__, __LINE__);
...
#pragma omp for schedule(static)
                for (i = ctx->ghost; i < ctx->NrTotal; ++i)
                {
                    ...
                    for (j = ctx->ghost; j < ctx->NzTotal; ++j)
                    {
                        ...
                        u[4 * ctx->dim + IDX(i, j)] =
                            ctx->psi0 * exp(-0.5 * r * r / (ctx->sigmaR * ctx->sigmaR)) *
                                exp(-0.5 * z * z / (ctx->sigmaZ * ctx->sigmaZ)) +
                            (ctx->psi0 * exp(-chi * rr) / pow(rr, ctx->l + 1)) *
                                (0.5 + 0.5 * erf(2.0 * (rr - ctx->rExt) / M_2_SQRTPI));
                    }
                }
            }
        }
        else
        {
            ...
            read_single_file_2d(u + 4 * ctx->dim, ctx->psi_i, ctx->NrTotal, ctx->NzTotal,
                                ctx->NrTotalInitial, ctx->NzTotalInitial, __FILE__, __LINE__);
...
#pragma omp for schedule(static)
                for (i = k; i < ctx->NrTotal; ++i)
                {
                    ...
                    for (j = ctx->ghost; j < ctx->NzTotal; ++j)
                    {
                        u[5 * ctx->dim + IDX(i, j)] = (exp(2.0 * u[3 * ctx->dim + IDX(i, j)]) -
        ...
        {
            read_single_file_2d(u + 5 * ctx->dim, ctx->lambda_i, ctx->NrTotal, ctx->NzTotal,
                                ctx->NrTotalInitial, ctx->NzTotalInitial, __FILE__, __LINE__);
    ...
    for (i = 0; i < ctx->ghost; ++i)
    {
        ...
        {
            u[0 * ctx->dim + IDX(i, j)] =
                u[0 * ctx->dim + IDX(2 * ctx->ghost - (i + 1), 2 * ctx->ghost - (j + 1))];
            u[1 * ctx->dim + IDX(i, j)] =
                u[1 * ctx->dim + IDX(2 * ctx->ghost - (i + 1), 2 * ctx->ghost - (j + 1))];
            u[2 * ctx->dim + IDX(i, j)] =
                u[2 * ctx->dim + IDX(2 * ctx->ghost - (i + 1), 2 * ctx->ghost - (j + 1))];
            u[3 * ctx->dim + IDX(i, j)] =
                u[3 * ctx->dim + IDX(2 * ctx->ghost - (i + 1), 2 * ctx->ghost - (j + 1))];
            u[4 * ctx->dim + IDX(i, j)] =
                u[4 * ctx->dim + IDX(2 * ctx->ghost - (i + 1), 2 * ctx->ghost - (j + 1))];
            u[5 * ctx->dim + IDX(i, j)] =
                u[5 * ctx->dim + IDX(2 * ctx->ghost - (i + 1), 2 * ctx->ghost - (j + 1))];
        }
    }
// Axis.
