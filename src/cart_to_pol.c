...
void ex_cart_to_pol(double **i_u, double **i_rr, double **i_th, double *r, double *z, double *u,
                    double *Dr_u, double *Dz_u, double *Drz_u, const MKL_INT g_num, const double dr,
                    const double dz, const MKL_INT NrInterior, const MKL_INT NzInterior,
                    const MKL_INT ghost, MKL_INT *p_NrrTotal, MKL_INT *p_NthTotal, MKL_INT *p_p_dim,
                    double *p_drr, double *p_dth, double *p_rr_inf)
{
...
printf("*** rr_inf = %lf\t drr = %lf\t dth = %lf\t NrrTotal = %lld\t NthTotal = %lld\t p_dim =
%lld\n", rr_inf, drr, dth, NrrTotal, NthTotal, p_dim); printf("*** Doing bicubic
interpolation...\n");
*/
...
#pragma omp parallel for schedule(dynamic, 1) private(i, j, aux_rr) shared(p_rr, p_th)
    ...
    for (k = 0; k < g_num; ++k)
    {
        ...
        aux_u = bicubic(ghost - 1, ghost - 1, 0.5, 0.5, u + k * dim, Dr_u + k * dim, Dz_u + k * dim,
...
#pragma omp parallel for schedule(dynamic, 1)                                                      \
    ...
    for (i = 1; i < NrrTotal; ++i)
    {
        ...
        for (j = 0; j < NthTotal; ++j)
        {
            ...
            for (k = 0; k < g_num; ++k)
            {
                p_u[k * p_dim + P_IDX(i, j)] =
                    bicubic(i0, j0, di, dj, u + k * dim, Dr_u + k * dim, Dz_u + k * dim,
                            Drz_u + k * dim, dr, dz, NrTotal, NzTotal);
            }
        }
    }
