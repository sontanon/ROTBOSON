...
MKL_INT
nleq_err(rb_context *ctx, // INPUT: Runtime context.
...
)
{
    ...
    printf("***** \n"
           ...
           "***** \n"
           "***** Original algorithm based on (2011) Peter Deuflhard, \"Newton Methods for "
           "Nonlinear Problems\".\n"
           "***** This C program was written by Santiago Ontanon Sanchez, ICN UNAM, Mexico City "
           "September 2019.\n"
           "***** \n");
    ...
    for (k = 0; k < max_newton_iterations; ++k)
    {
        ...
        if (k % 50 == 0)
        {
            printf("*****  ------------ -------------- -------------- -------------- "
                   "-------------- ------------- \n"
                   "***** | NLEQ ITER  | ||du[k]||    | lambda[k]    | Theta[k]     | lambda'[k]   "
                   "| STATUS      |\n"
                   "***** "
                   "|------------|--------------|--------------|--------------|--------------|-----"
                   "--------|\n");
...
#endif
        {
            ...
            printf("*****  ------------ -------------- -------------- -------------- "
                   "-------------- ------------- \n"
                   "***** \n"
                   "***** NLEQ-ERR Algorithm converged successfully after %lld iterations. "
                   "Converged on ||du||.\n"
                   "***** \n"
        ...
        if (k > prediction_start)
        {
            ...
            mu[k] = (norm_du[k - 1] * norm_du_bar[k] * lambda[k - 1]) /
                    (norm_du_bar_minus_du * norm_du[k]);
    ...
    REGULARITY_TEST:
        if (lambda[k] < lambda_min)
        {
            ...
            printf("*****  ------------ -------------- -------------- -------------- "
                   ...
                   "***** Reason for failure is that damping factor %11.5E is smaller that minimum "
                   "specified %11.5E.\n"
                   "***** \n"
                   "***** Will exit after cleanup... \n"
                   "***** \n",
    ...
    TRIAL_ITERATE:
        ARRAY_SUM(u[k + 1], 1.0, u[k], lambda[k], du[k]);
        ...
        mu_prime[k] =
            0.5 * norm_du[k] * lambda[k] * lambda[k] / norm_du_bar_minus_one_minus_lambda_du;

        // If Theta_k > 1 - lambda_k / 4:
        // then replace lambda_k by lambda_prime_k = min(mu_prime_k, lambda_k / 2). Go to regularity
        // test.
        ...
        if (Theta[k] > THETA_MAX - RESTRICTED * 0.25 * lambda[k])
        {
            ...
            printf("***** | %-10lld | %-12.5E | %-11.5E  | %-12.5E | %-11.5E  | %-11s |\n", k,
                   norm_du[k] / norm_u, lambda[k], Theta[k], lambda_prime[k], "REJECT A");
            ...
            /* Else we have exceeded max_trail_A_iterations, which might indicate that we are stuck
             * in a loop. */
            ...
            else
            {
                ...
                printf("*****  ------------ -------------- -------------- -------------- "
                       ...
                       "***** \n"
                       "***** NLEQ-ERR Algorithm failed after %lld iterations.\n"
                       "***** Reason for failure is that A type lambda_k replacement by "
                       "lambda_prime_k has undergone a maximum of %lld iterations.\n"
                       "***** \n"
                       "***** Will exit after cleanup... \n"
                       "***** \n",
        ...
        else
        {
            ...
            /* Reset l_A trial counter since we have found a proper lambda factor that makes
             * Theta[k] < 1 - lambda[k] / 4. */
            l_A = 0;
        }
        ...
        {
            // Convergence test: If ||du_bar^{k+1}|| < epsilon: stop. Solution found u* = u^{k+1} +
            // du_bar^{k+1}.
#ifdef NORMALIZED
...
#endif
            {
                ...
                printf("***** | %-10lld | %-12.5E | %-11.5E  | %-12.5E | %-11.5E  | %-11s |\n", k,
                       norm_du_bar[k + 1] / norm_u, lambda[k], Theta[k], lambda_prime[k],
                       "CONVERGED B");
                printf("*****  ------------ -------------- -------------- -------------- "
                       "-------------- ------------- \n"
                       "***** \n"
            ...
            if (Theta[k] < 0.5)
            {
                ...
                {
                    printf("***** | %-10lld | %-12.5E | %-11.5E  | %-12.5E | %-11.5E  | %-11s |\n",
                           k, norm_du_bar[k + 1] / norm_u, lambda[k], Theta[k], lambda_prime[k],
                           "ENTER QNERR");

                    /* Call QNERR with initial iterates u[k+1], f[k+1]. J(u[k+1]) will be calculated
                     * inside QNERR. */
                    /* Theta will remain the monitoring quantity, whereas mu is the alpha parameter
                     * inside QNERR. */
                    /* At this point, we have done k + 1 iterations, so QNERR is called with that
                     * number less iterations. */
                    /* qnerr_stop is the returned integer: it will be positive if QNERR succeeded,
                     * but negative otherwise. */
                    qnerr_stop = nleq_err_qnerr(
                        ctx, &qnerr_code, u + k + 1, f + k + 1, du + k + 1, du_bar + k + 1,
                        norm_du + k + 1, norm_du_bar + k + 1, Theta + k + 1, mu + k + 1, J, epsilon,
                        max_newton_iterations - (k + 1), RHS_CALC, JACOBIAN_CALC, NORM, DOT,
                    ...
                    if (qnerr_code == ERROR_CODE_SUCCESS)
                    {
                        ...
                        printf("***** \n"
                               "***** NLEQ-ERR Algorithm converged successfully after %lld "
                               "iterations. Converged on QNERR.\n"
                               "***** \n"
                               "***** Will exit after cleanup... \n"
                               "***** \n",
                               k + 1 + qnerr_stop);
                    ...
                    {
                        printf("***** | NLEQ ITER  | ||du[k]||    | lambda[k]    | Theta[k]     | "
                               "lambda'[k]   | STATUS      |\n"
                               "***** "
                               "|------------|--------------|--------------|--------------|--------"
                               "------|-------------|\n");
                        ...
                        /* In this case, the last update is stored in u[k + 1 - qnerr_stop] since
                         * qnerr_stop is negative. */
                        /* Therefore, this will function as the initial iteration for NLEQ. We need,
                         * however, an initial */
                        /* lambda damping factor which will be set to 1.0. */
                        lambda[k + (-qnerr_stop) + 1] = LOCAL_LAMBDA;

                        /* Set k to one place before last update. */
                        /* This k is not where the last solution is stored, but, rather, one place
                         * before. */
                        /* This is due to the fact that the "continue" statement below will increase
                         * k via */
                        /* the ++k statement at the for loop above. */
                        k += (-qnerr_stop);

                        /* Increase prediction start. */
                        /* This is because we do not have a lambda[k - 1] and have to start a
                         * prediction again. */
                        prediction_start = k + 1;

                        /* Goto 1. */
                        /* Notice that if we have reached max_newton_iterations, we will immediately
                         * go to the */
                    ...
                    {
                        printf("***** \n"
                               "***** NLEQ-ERR Algorithm failed to converge after %lld maximum "
                               "number of iterations. Failure on QNERR.\n"
                               "***** \n"
                               "***** Will exit after cleanup... \n"
                               "***** \n",
        ...
        {
            /* Print message. Iterate rejected because lambda_prime is bigger than lambda by at
             * leaste a factor of 4. */
            printf("***** | %-10lld | %-12.5E | %-11.5E  | %-12.5E | %-11.5E  | %-11s |\n", k,
                   norm_du[k] / norm_u, lambda[k], Theta[k], lambda_prime[k], "REJECT B");
            ...
            /* Else we have exceeded max_trail_B_iterations, which might indicate that we are stuck
             * in a loop. */
            /* Therefore, exit to be safe. */
            else
            {
                ...
                printf("*****  ------------ -------------- -------------- -------------- "
                       ...
                       "***** \n"
                       "***** NLEQ-ERR Algorithm failed after %lld iterations.\n"
                       "***** Reason for failure is that B type lambda_k replacement by "
                       "lambda_prime_k has undergone a maximum of %lld iterations.\n"
                       "***** \n"
                       "***** Will exit after cleanup... \n"
                       "***** \n",
        ...
        {
            /* Print message. Iterate accepted because Theta is not too big; we have not reached
             * safe region; and lambda_prime is not too big. */
            printf("***** | %-10lld | %-12.5E | %-11.5E  | %-12.5E | %-11.5E  | %-11s |\n", k,
                   norm_du[k] / norm_u, lambda[k], Theta[k], lambda_prime[k], "ACCEPT");
