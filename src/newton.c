...
MKL_INT newton(rb_context *ctx, // INPUT: Runtime context.
    ...
    for (k = 0; k < max_newton_iterations; ++k)
    {
        ...
        if (norm_du[k] < epsilon)
        {
            ...
            printf("***** | %-10lld | %-12.5E |              |              | %-11s |\n", k,
                   norm_du[k], "CONVERGED A");
            printf(
                "*****  ------------ -------------- -------------- -------------- ------------- \n"
                "***** \n"
                "***** NCER-ERR Algorithm converged successfully after %lld iterations. Converged "
                "on ||du||.\n"
                "***** \n"
        ...
        /* Print message. Iterate accepted because Theta is not too big; we have not reached safe
         * region; and lambda_prime is not too big. */
        printf("***** | %-10lld | %-12.5E | %-11.5E  | %-12.5E | %-11s |\n", k, norm_du[k],
               lambda[k], Theta[k], "ACCEPT");
    }
