...
def main() -> None:
    ...
    ok_s, lines = compare_scalars(extract_scalars(ref), extract_scalars(new), rtol=args.rtol)
    ok &= ok_s
    print("\n".join(lines))

    print("\nField profiles:")
    ok_f, lines = compare_fields(ref, new, rtol=args.rtol)
    ok &= ok_f
    print("\n".join(lines))

    verdict = "PASS" if ok else "FAIL"
    print(f"\nComparison verdict: {verdict} (rtol={args.rtol})")
