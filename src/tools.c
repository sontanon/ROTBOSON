...
void array_sum(double *z, const double alpha, double *x, const double beta, double *y,
               const MKL_INT dim)
{
    MKL_INT i;
...
void safe_free(void *x, const char *file, const MKL_INT line)
{
    if (!x)
    {
        fprintf(
            stderr,
            "WARNING! Memory block attempting to free already points to NULL on file %s:%lld.\n",
            file, line);
    }
    else
    {
...
void write_single_file_2d(double *u, const char *fname, const MKL_INT NrTotal,
...
void write_iterated_file_2d(double **u, const char *fname, const MKL_INT NrTotal,
                            const MKL_INT NzTotal, const MKL_INT iterations, const MKL_INT gnum)
{
    // Auxiliary integers.
    MKL_INT i, j, k;
...
void write_single_file_2d_polar(double *u, const char *fname, const MKL_INT NrrTotal,
                                const MKL_INT NthTotal)
{
    // Auxiliary integers.
    MKL_INT i, j;
...
void read_single_file_1d(double *u, const char *fname, const MKL_INT dim, const char *source_file,
                         const MKL_INT source_line)
{
    // Auxiliary integer.
    MKL_INT i, err;
...
void read_single_file_2d(double *u, const char *fname, const MKL_INT NrTotal, const MKL_INT NzTotal,
                         const MKL_INT NrTotalInitial, const MKL_INT NzTotalInitial,
                         const char *source_file, const MKL_INT source_line)
{
    // Auxiliary integers.
    MKL_INT i, j, err;
    ...
    if (NzTotalInitial < NzTotal || NrTotalInitial < NrTotal)
    {
        ...
        for (i = NrTotalInitial; i < NrTotal; ++i)
        {
            ...
            {
                u[IDX(i, j)] =
                    0.5 * (u[IDX(i, NzTotalInitial - 1)] + u[IDX(NrTotalInitial - 1, j)]);
            }
        }
    }
