...
void nnz_jacobian_get_nnzs(MKL_INT *p_nnz1, MKL_INT *p_nnz2, MKL_INT *p_nnz3, MKL_INT *p_nnz4, MKL_INT *p_nnz5, MKL_INT *p_nnz6, MKL_INT *p_nnz7, MKL_INT *p_nnz8)
{
	MKL_INT nnz1 = 0, nnz2 = 0, nnz3 = 0, nnz4 = 0, nnz5 = 0;
	MKL_INT nnz6 = 0, nnz7 = 0, nnz8 = 0;
	...
	if (order == 4)
	{
		...
		nnz5 = 31 * NrInterior * NzInterior 
			...
			+ 4 * (NrInterior + NzInterior) + 24;
		nnz6 = 52 * NrInterior * NzInterior
			+ 53 * NrInterior + 55 * NzInterior + 56
			+ 4 * (NrInterior + NzInterior) + 24;
		nnz7 = 5 * NrInterior * NzInterior
			+ 5 * NrInterior + 6 * NzInterior + 6
			+ 4 * (NrInterior + NzInterior) + 24;
		nnz8 = 5 * NrInterior * NzInterior
			+ 5 * NrInterior + 6 * NzInterior + 6 
			+ 4 * (NrInterior + NzInterior) + 24;
		...
		nnz7 += 5 * NrInterior + 6 * NzInterior + 6 * 3;
		nnz8 += 5 * NrInterior + 6 * NzInterior + 6 * 3;
	}
	else
	{
		// Interior points plus parity boundaries.
		...
		nnz7 += 3 * NrInterior + 4 * NzInterior + 4;
		nnz8 += 3 * NrInterior + 4 * NzInterior + 4;
	}
...
MKL_INT nnz_jacobian(void)
{
	...
	MKL_INT nnz1 = 0, nnz2 = 0, nnz3 = 0, nnz4 = 0, nnz5 = 0, nnz6 = 0, nnz7 = 0, nnz8 = 0;

	nnz_jacobian_get_nnzs(&nnz1, &nnz2, &nnz3, &nnz4, &nnz5, &nnz6, &nnz7, &nnz8);
#ifdef DEBUG
	printf("nnz1 = %lld,\n", nnz1);
	printf("nnz2 = %lld,\n", nnz2);
	printf("nnz3 = %lld,\n", nnz3);
	...
	return nnz1 + nnz2 + nnz3 + nnz4 + nnz5 + nnz6 + nnz7 + nnz8 + 1;
}
...
void csr_gen_jacobian(csr_matrix A, double *u, const int print)
{
	...
	// Integer arrays.
	MKL_INT r_sym[GNUM] = {EVEN, EVEN, EVEN, EVEN, EVEN, EVEN, EVEN, EVEN};
	MKL_INT z_sym[GNUM] = {EVEN, EVEN, EVEN, EVEN, EVEN, EVEN, EVEN, EVEN};
	MKL_INT bound_order[GNUM] = {ROBIN_TYPE_1, ROBIN_TYPE_1, ROBIN_TYPE_1, ROBIN_TYPE_1,
		EXP_DECAY_TYPE, ROBIN_TYPE_1, REG_VARIABLE, REG_VARIABLE};
	MKL_INT nnzs[GNUM] = {nnz1, nnz2, nnz3, nnz4, nnz5, nnz6, nnz7, nnz8};

	MKL_INT p_cc[GNUM] = {0, 0, 0, 0, 0, 0, 0, 0};
	MKL_INT p_cs[GNUM] = {0, 0, 0, 0, 0, 0, 0, 0};
	MKL_INT p_sc[GNUM] = {0, 0, 0, 0, 0, 0, 0, 0};
	MKL_INT p_ss[GNUM] = {0, 0, 0, 0, 0, 0, 0, 0};
	MKL_INT p_bound[GNUM] = {0, 0, 0, 0, 0, 0, 0, 0};
