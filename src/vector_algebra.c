...
{
	double sum = cblas_ddot(ctx->NzTotal * ctx->NrInterior, x + ctx->ghost * ctx->NzTotal, 1, x + ctx->ghost * ctx->NzTotal, 1);

	MKL_INT k = 0;

	for (k = 0; k < ctx->ghost; ++k)
	{
		sum -= cblas_ddot(ctx->NrInterior, x + ctx->ghost * ctx->NzTotal + k, ctx->NzTotal, x + ctx->ghost * ctx->NzTotal + k, ctx->NzTotal);
		sum -= cblas_ddot(ctx->NrInterior, x + (ctx->ghost + 1) * ctx->NzTotal - ctx->ghost + k, ctx->NzTotal, x + (ctx->ghost + 1) * ctx->NzTotal - ctx->ghost + k, ctx->NzTotal);
	}
...
{
	double sum = cblas_ddot(ctx->NzTotal * ctx->NrInterior, x + ctx->ghost * ctx->NzTotal, 1, y + ctx->ghost * ctx->NzTotal, 1);

	MKL_INT k = 0;

	for (k = 0; k < ctx->ghost; ++k)
	{
		sum -= cblas_ddot(ctx->NrInterior, x + ctx->ghost * ctx->NzTotal + k, ctx->NzTotal, y + ctx->ghost * ctx->NzTotal + k, ctx->NzTotal);
		sum -= cblas_ddot(ctx->NrInterior, x + (ctx->ghost + 1) * ctx->NzTotal - ctx->ghost + k, ctx->NzTotal, y + (ctx->ghost + 1) * ctx->NzTotal - ctx->ghost + k, ctx->NzTotal);
	}
