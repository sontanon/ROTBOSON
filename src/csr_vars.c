...
)
{
	...
	aa[offset1 +  2] = (D_2_21)*(dzodr + drodz) + r2h2oalpha2dOmega2 
	...
	aa[offset1 + 17] = dw_du(chi, m) * (-16.0*M_PI*dr2*dzodr*a2*wplOmega*phi2/alpha2);
	...
	aa[offset2 + 16] = dw_du(chi, m) * (-16.0*M_PI*dr2*dzodr*a2*l*phi2or2/h2);
	...
	aa[offset3 + 11] = (D_2_21)*(drodz + dzodr) + r2h2oalpha2dOmega2 
	...
	aa[offset4 + 25] = dw_du(chi, m) * (8.0*M_PI*dr2*dzodr*a2*wplOmega*phi2/alpha2);
	...
	aa[offset5 + 14] = (D_2_21)*(drodz + dzodr) + dzodr * (l * (dRu1 / ri + dRu3 / ri) 
	...
	aa[offset5 + 17] = dw_du(chi, m) * (2.0*dr2*dzodr*a2*wplOmega*psi/alpha2);
...
)
{
	...
	aa[offset5 + 26] = (D22)*drodz + (S24)*dzodr + dzodr*((S14)*((2.0*l + 1.0)/ri + 2.0*dRu5 + dRu1 + dRu3 - 2.0*chi*dr2*ri/rr));
...
)
{
	...
	aa[offset4 +  6] = dzodr*((S14)*(-1.0/ri - dRu3)) + drodz*((S14)*(-dZu3)) + 0.5*r2h2oalpha2dOmega2 - 8.0*M_PI*dr2*dzodr*a2*wplOmega2*phi2/alpha2;
	...
	aa[offset5 + 28] = (S24)*drodz + drodz*((S14)*(2.0*dZu5 + dZu1 + dZu3 - 2.0*chi*dz2*zi/rr)) + (S24)*dzodr + dzodr*((S14)*((2.0*l + 1.0)/ri + 2.0*dRu5 + dRu1 + dRu3 - 2.0*chi*dr2*ri/rr));
