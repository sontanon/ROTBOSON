...
)
{
    ...
    for (l = 0; l < max_newton_iterations; ++l)
    {
        ...
        if (l % 50 == 0)
        {
            printf("*****  ------------ -------------- -------------- -------------- "
                   "-------------- ------------- \n"
                   "***** | QNERR ITER | ||du[l+1]||  | alpha_bar    | Theta[l]     | alpha[l+1]   "
                   "| STATUS      |\n"
                   "***** "
                   "|------------|--------------|--------------|--------------|--------------|-----"
                   "--------|\n");
        ...
        if (Theta[l] > 0.5 && (l + 1) > MIN_ITERATIONS)
        {
            ...
            printf("***** | %-10lld |              | % 11.5E | % -9.5E | % 11.5E | %-11s |\n", l,
                   alpha_bar, Theta[l], alpha[l + 1], "EXIT QNERR");
            printf("*****  ------------ -------------- -------------- -------------- "
                   "-------------- ------------- \n");
        ...
        // Convergence test: If ||du^{l+1}|| < epsilon: stop. Solution found u* = u^{l+1} +
        // du^{l+1}.
#ifdef NORMALIZED
...
#endif
        {
            ...
            printf("***** | %-10lld | % -9.5E | % 11.5E | % -9.5E | % 11.5E | %-11s |\n", l,
                   norm_du[l + 1] / norm_u, alpha_bar, Theta[l], alpha[l + 1], "CONVERGED C");
            printf("*****  ------------ -------------- -------------- -------------- "
                   "-------------- ------------- \n");
        ...
        printf("***** | %-10lld | % -9.5E | % 11.5E | % -9.5E | % 11.5E | %-11s |\n", l,
               norm_du[l + 1] / norm_u, alpha_bar, Theta[l], alpha[l + 1], "ACCEPT");
    }
