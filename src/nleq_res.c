...
MKL_INT
nleq_res(rb_context *ctx, // INPUT: Runtime context.
...
)
{
    ...
    for (k = 0; k < max_newton_iterations; ++k)
    {
        ...
        if (k % 50 == 0)
        {
            printf("*****  ------------ -------------- ------------- -------------- ------------- "
                   "------------- \n"
                   "***** | NLEQ ITER  | ||f[k]||     | lambda[k]   | Theta[k]     | lambda'[k]  | "
                   "STATUS      |\n"
                   "***** "
                   "|------------|--------------|-------------|--------------|-------------|-------"
                   "------|\n");
        ...
        if (norm_f[k] < epsilon)
        {
            ...
            printf("*****  ------------ -------------- ------------- -------------- ------------- "
                   "------------- \n"
                   "***** \n"
                   "***** NLEQ-RES Algorithm converged successfully after %lld iterations. "
                   "Converged on ||f||.\n"
                   "***** \n"
    ...
    TRIAL_ITERATE:
        ...
        if (Theta[k] >= 1.0)
        {
            ...
            printf("***** | %-10lld | %-12.5E | %-11.5E | %-12.5E | %-11.5E | %-11s |\n", k,
                   norm_f[k], lambda[k], Theta[k], lambda_prime[k], "REJECT A");
        ...
        if (lambda_prime[k] == lambda[k] && lambda[k] == 1.0 && Theta[k] < THETA_MAX)
        {
            ...
            {
                printf("***** | %-10lld | %-12.5E | %-11.5E | %-12.5E | %-11.5E | %-11s |\n", k,
                       norm_f[k], lambda[k], Theta[k], lambda_prime[k], "ENTER QNRES");

                // Call QNRES with initial iterates u[k+1], f[k+1], ||f[k+1]||. J(u[k+1]) will be
                // calculated inside QNRES. Theta will remain the monitoring quantity, whereas mu is
                // the gamma parameter inside QNERR. At this point we have done k + 1 iterations, so
                // QNRES is called with that number less iterations. qnres_stop is the returned
                // integer: it will be positive if QNRES succeeded, but negative otherwise.
                qnres_stop = nleq_res_qnres(
                    ctx, &qnres_code, u + k + 1, f + k + 1, du + k + 1, norm_f + k + 1,
                    Theta + k + 1, mu + k + 1, J, epsilon, max_newton_iterations - (k + 1),
                ...
                else if (qnres_code == ERROR_CODE_QNRES_THETA_INCREASE_EXIT ||
                         qnres_code == ERROR_CODE_QNRES_ILL_CONDITIONED)
                {
                    ...
                    /* In this case, the last update is stored in u[k + 1 - qnres_stop] since
                     * qnres_stop is negative. */
                     ...
                     * before. */
                    /* This is due to the fact that the "continue" statement below will increase k
                     * via */
                    /* the ++k statement at the for loop above. */
                    k += (-qnres_stop);

                    /* Increase prediction start. */
                    /* This is because we do not have a lambda[k - 1] and have to start a prediction
                     * again. */
                    prediction_start = k + 1;

                    /* Goto 1. */
                    /* Notice that if we have reached max_newton_iterations, we will immediately go
                     * to the */
                ...
                {
                    printf("***** \n"
                           "***** NLEQ-ERR Algorithm failed to converge after %lld maximum number "
                           "of iterations. Failure on QNERR.\n"
                           "***** \n"
                           "***** Will exit after cleanup... \n"
                           "***** \n",
            ...
            else
            {
                ...
                printf("***** | %-10lld | %-12.5E | %-11.5E | %-12.5E | %-11.5E | %-11s |\n", k,
                       norm_f[k], lambda[k], Theta[k], lambda_prime[k], "ACCEPT");
        ...
        else if (lambda_prime[k] > 4.0 * lambda[k])
        {
            ...
            printf("***** | %-10lld | %-12.5E | %-11.5E | %-12.5E | %-11.5E | %-11s |\n", k,
                   norm_f[k], lambda[k], Theta[k], lambda_prime[k], "REJECT B");
