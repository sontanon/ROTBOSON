    ...
    *f_res = l_res + r_res;
    ...
    if (print)
    {
        ...
        printf(
            "***  -------------------------- ----------------------- ----------------------- \n");
        printf(
            "*** | psi(0)                   | psi(rr(max(phi))      | HWL Resolution        |\n");
        printf(
            "***  -------------------------- ----------------------- ----------------------- \n");
...
#pragma omp parallel for schedule(dynamic, 1) shared(i0, i1, i2) private(k)
    for (k = 0; k < p_dim; ++k)
    {
        i0[k] = (exp(sph_log_alpha[k]) * sph_Drr_log_alpha[k] -
                 0.5 * exp(2.0 * sph_log_h[k]) * sph_rr[k] * sph_rr[k] * sin(sph_th[k]) *
                     sin(sph_th[k]) * sph_beta[k] * sph_Drr_beta[k] / exp(sph_log_alpha[k])) *
                exp(sph_log_h[k]) * sph_rr[k] * sph_rr[k] * sin(sph_th[k]);
        i1[k] = -0.25 *
                (exp(2.0 * sph_log_a[k]) * (2.0 * sph_rr[k] * sph_Drr_log_a[k] - 1.0) +
                 exp(2.0 * sph_log_h[k]) * (2.0 * sph_rr[k] * sph_Drr_log_h[k] + 1.0)) *
                sph_rr[k] * sin(sph_th[k]);
        i2[k] = 0.25 * exp(3.0 * sph_log_h[k]) * sph_rr[k] * sph_rr[k] * sph_rr[k] * sph_rr[k] *
...
#pragma omp parallel for schedule(dynamic, 1) shared(i0, i1) private(k)
    ...
    {
        i0[k] = 4.0 * M_PI *
                (2.0 * w * (w + l * sph_beta[k]) / exp(sph_log_alpha[k]) -
                 exp(sph_log_alpha[k]) * m * m) *
                (sph_psi[k] * sph_psi[k] * pow(sph_rr[k] * sin(sph_th[k]), 2 * l)) *
                exp(2.0 * sph_log_a[k] + sph_log_h[k]) * sph_rr[k] * sph_rr[k] * sin(sph_th[k]);
        i1[k] = 4.0 * M_PI * l * (w + l * sph_beta[k]) *
                (sph_psi[k] * sph_psi[k] * pow(sph_rr[k] * sin(sph_th[k]), 2 * l)) *
                exp(2.0 * sph_log_a[k] + sph_log_h[k]) * sph_rr[k] * sph_rr[k] * sin(sph_th[k]) /
                exp(sph_log_alpha[k]);
    }
...
#pragma omp parallel for schedule(dynamic, 1) shared(i0, i1, i2, i3)                               \
    ...
    for (k = NthTotal; k < p_dim; ++k)
    {
        ...
        i0[k] =
            4.0 * M_PI * aux_rr *
            (aux_a2 *
                 (((w + l * aux_beta) * (w + l * aux_beta) / aux_alpha2 - m * m) * aux_r * aux_r +
                  l * l / aux_h2) *
                 aux_phi2_o_r2 -
             (l * l * aux_phi2_o_r2 +
              aux_rlm1 * aux_rlm1 * sin(aux_th) * sin(aux_th) *
                  ((aux_rr * sph_Drr_psi[k]) * (aux_rr * sph_Drr_psi[k]) +
                   sph_Dth_psi[k] * sph_Dth_psi[k]) +
              2.0 * l * aux_phi_o_r * aux_rlm1 * sin(aux_th) *
                  (sin(aux_th) * (aux_rr * sph_Drr_psi[k]) + cos(aux_th) * sph_Dth_psi[k])));
        // 0.75 * H**2 * rr * (r**2 * D_beta_D_beta) / alpha**2.
        i1[k] = 0.75 * aux_h2 * aux_rr * sin(aux_th) * sin(aux_th) *
                ((aux_rr * sph_Drr_beta[k]) * (aux_rr * sph_Drr_beta[k]) +
                 sph_Dth_beta[k] * sph_Dth_beta[k]) /
                aux_alpha2;
        // -r * D_log_alpha_D_log_alpha.
        i2[k] = -(sph_Drr_log_alpha[k] * (aux_rr * sph_Drr_log_alpha[k]) +
                  sph_Dth_log_alpha[k] * (sph_Dth_log_alpha[k] / aux_rr));
        // Sum all components.
        i3[k] = i0[k] + i1[k] + i2[k];
    }
...
#pragma omp parallel for schedule(dynamic, 1) shared(i0, i1, i2, i3)                               \
    ...
    for (k = NthTotal; k < p_dim; ++k)
    {
        ...
        i0[k] =
            4.0 * M_PI * exp(sph_log_h[k]) * aux_rr * aux_rr * sin(aux_th) *
            (aux_a2 *
                 (1.5 * ((w + l * aux_beta) * (w + l * aux_beta) / aux_alpha2 - m * m) * aux_r *
                      aux_r -
                  0.5 * l * l / aux_h2) *
                 aux_phi2_o_r2 -
             0.5 * (l * l * aux_phi2_o_r2 +
                    aux_rlm1 * aux_rlm1 * sin(aux_th) * sin(aux_th) *
                        ((aux_rr * sph_Drr_psi[k]) * (aux_rr * sph_Drr_psi[k]) +
                         sph_Dth_psi[k] * sph_Dth_psi[k]) +
                    2.0 * l * aux_phi_o_r * aux_rlm1 * sin(aux_th) *
        ...
        i1[k] = 0.375 * exp(3.0 * sph_log_h[k]) * sin(aux_th) * sin(aux_th) * sin(aux_th) * aux_rr *
                aux_rr *
                ((aux_rr * sph_Drr_beta[k]) * (aux_rr * sph_Drr_beta[k]) +
                 sph_Dth_beta[k] * sph_Dth_beta[k]) /
                aux_alpha2;
        // Metric derivative terms.
        i2[k] = -sin(aux_th) * exp(sph_log_h[k]) *
                    (((aux_rr * sph_Drr_log_alpha[k]) * (aux_rr * sph_Drr_log_alpha[k]) +
                      sph_Dth_log_alpha[k] * sph_Dth_log_alpha[k]) -
                     0.5 * ((aux_rr * sph_Drr_log_h[k]) * (aux_rr * sph_Drr_log_a[k]) +
                            sph_Dth_log_h[k] * sph_Dth_log_a[k])) +
                0.5 * (aux_h2 - aux_a2) *
                    (sin(aux_th) * (aux_rr * sph_Drr_log_a[k]) + cos(aux_th) * sph_Dth_log_a[k] -
                     0.5 * (sin(aux_th) * (aux_rr * sph_Drr_log_h[k]) +
                            cos(aux_th) * sph_Dth_log_h[k])) /
                    exp(sph_log_h[k]);
    ...
    *J = 0.5 * (J_Komar1[NrrTotal - 1] + J_Komar2[NrrTotal - 1]);
    ...
    for (k = NrrTotal - 2; k > 0; --k)
    {
        if (0.5 * (M_Komar1[k] + M_Komar2[k]) < 0.99 * *M)
        {
            ...
            r99 = sph_rr[P_IDX(k, 0)] +
                  drr * (0.99 * *M - 0.5 * (M_Komar1[k] + M_Komar2[k])) /
                      (0.5 * (M_Komar1[k + 1] + M_Komar2[k + 1] - M_Komar1[k] - M_Komar2[k]));
            break;
        }
    }
    ...
    GRV2_c = -M_PI * (*M / rr_inf) * (*M / rr_inf) *
             (0.5 + (*M / rr_inf) *
                        ((4.0 / 3.0) +
                         (*M / rr_inf) *
                             (3.0 - (33.0 / 8.0) * a * a / (*M * *M) +
                              (*M / rr_inf) * ((32.0 / 5.0) - (31.0 / 5.0) * a * a / (*M * *M)))));

    GRV3_c = M_PI * *M * (*M / rr_inf) *
             (4.0 +
              (*M / rr_inf) *
                  (8.0 + (*M / rr_inf) *
                             (8.0 * (86.0 - 15.0 * a * a / (*M * *M)) / 45.0 +
                              (*M / rr_inf) *
                                  (2.0 * (1526.0 - 379.0 * a * a / (*M * *M)) / 105.0 +
                                   (*M / rr_inf) * (4.0 *
                                                    (21576.0 - 9256.0 * a * a / (*M * *M) +
                                                     1365.0 * a * a * a * a / (*M * *M * *M * *M)) /
                                                    1575.0)))));
    ...
    for (k = 0; k < p_dim; ++k)
    {
        ...
        i0[k] = exp(2.0 * sph_log_alpha[k]) - sph_rr[k] * sph_rr[k] * sin(sph_th[k]) *
                                                  sin(sph_th[k]) * exp(2.0 * sph_log_h[k]) *
                                                  sph_beta[k] * sph_beta[k];
    ...
    if (print)
    {
        ...
        printf("***  -------------------------- ----------------------- ----------------- "
               "----------------- \n");
        printf("*** | Komar M Geometry Surface | Komar M Matter Volume |      ADM M      | "
               "Schwarzschild M |\n");
        printf("***  -------------------------- ----------------------- ----------------- "
               "----------------- \n");
        // printf("*** |      1234567890123       |     1234567890123     |  1234567890123  |
        // 1234567890123  |      1234567890123       |     1234567890123      |\n");
        printf("*** |       %-6.5e        |      %-6.5e      |   %-6.5e   |   %-6.5e   |\n",
               M_Komar1[NrrTotal - 1], M_Komar2[NrrTotal - 1], M_ADM[NrrTotal - 1],
               M_Schwarz[NrrTotal - 1]);
        printf("***  -------------------------- ----------------------- ----------------- "
               "----------------- \n");
        printf("*** \n");
        printf("***  -------------------------- ------------------------ \n");
