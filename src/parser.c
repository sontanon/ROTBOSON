...
void parser(rb_context *ctx, const char *fname)
{
	...
	{
		if (ctx->readInitialData != 0 && ctx->readInitialData != 1 && ctx->readInitialData != 2 && ctx->readInitialData != 3)
	...
	case 3:
		...
		if (lookup_int(tab, "NrTotalInitial", &ctx->NrTotalInitial))
			check_int_in_range("NrTotalInitial", ctx->NrTotalInitial, MIN_NRINTERIOR, MAX_NRINTERIOR);
		else
			die("readInitialData = 3 requires \"NrTotalInitial\".\n");

		if (lookup_int(tab, "NzTotalInitial", &ctx->NzTotalInitial))
			check_int_in_range("NzTotalInitial", ctx->NzTotalInitial, MIN_NRINTERIOR, MAX_NRINTERIOR);
		else
	...
	{
		if (ctx->rr_phi_max_minimum < 4 * ctx->dr || ctx->rr_phi_max_minimum > ctx->dr * ctx->NrInterior)
	...
	{
		if (ctx->rr_phi_max_maximum < ctx->rr_phi_max_minimum || ctx->rr_phi_max_maximum > ctx->dr * ctx->NrTotal)
	...
	snprintf(ctx->initial_dirname, MAX_STR_LEN, "l=%lld,w=X.XXXXXE-01,dr=%.5E,N=%04lld", ctx->l, ctx->dr, ctx->NrInterior);
